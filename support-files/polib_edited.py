    ...
    def __unicode__(self, wrapwidth=78):
        ...
        if not self.obsolete and self.occurrences:
            ...
            if self.occurrences == "None":  # Vrstica prazna..ipd
                ...
                reformatted_string_temp = (self.occurrences).split(",")    # Razdeli na ločene pathe+lineno
                for index_temp in range(len(reformatted_string_temp)):
                        reformatted_string_temp[index_temp] = (reformatted_string_temp[index_temp].strip()).rsplit(":", 1)

                # Poskusi razpakirati posamezen del source komentarja na path in line number.
                # Če ne zna, obvesti in pusti prazno. Tukaj začnejo delat probleme presledki v pathu pri generaciji xlsx tabele.
                try:
                    for fpath, lineno in reformatted_string_temp:  # Zapiši v filelist
                        if lineno:
                            filelist.append('%s:%s' % (fpath, lineno))
                        else:
                            filelist.append(fpath)
                except:
                    print("Cant unpack the following line: " + self.occurrences + "\nLeaving an empty entry instead.")
                    filelist.append("")
